...
The `sf` package has a few dependencies of its own (not all of which
...
When installing packages from source (as is common on Linux), compilation errors may occur due to
aggressive compiler flag settings used in conjunction with C or C++ sources and `Rcpp`.
...
You should absolutely not go into `$(R RHOME)/etc/Makeconf` and change the compiler flags, like for example
removing `-Werror=format-security` from the `CXX14FLAGS` or similar ;)
